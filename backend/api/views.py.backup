...
from .serializers import (
    ...
    ProdutosMonitoradosExternosSerializer # Importar o novo serializer

)

from scraper.tasks import run_scraping_pipeline # Importar a tarefa Celery
...
class LongPathScrapeView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Aciona a tarefa Celery em segundo plano

        task = run_long_path_scrape.delay(url=url_to_scrape, user_id=user_id)



        # Retorna uma resposta imediata com o ID da tarefa

        return Response(

            {"message": "Requisição de scraping recebida. O processo está executando em segundo plano.", "task_id": task.id},
...
class ScrapeURLView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Fast Path: Usa requests para obter o HTML e BeautifulSoup para parsear
            ...
            scraped_data['title'] = soup.title.string.strip() if soup.title else "Nenhum título encontrado"
            ...
            logger.error(f"Erro de rede ao raspar a URL {url_to_scrape}: {e}")

            return Response(

                {"error": f"Não foi possível acessar a URL. Verifique o link ou tente novamente."},
            ...
            logger.error(f"Erro ao processar a página {url_to_scrape}: {e}")

            return Response(

                {"error": f"Ocorreu um erro ao processar a página: {e}"},
...
class MonitoramentoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Garante que o usuário só veja seus próprios produtos monitorados
        ...
        if self.request.user.is_authenticated:
            ...
            try:
                ...
                return ProdutosMonitoradosExternos.objects.none() # Retorna queryset vazia se não for vendedor
...
class IniciarMonitoramentoView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        task = run_scraping_pipeline.delay(url=url_to_scrape, user_id=user_id)
...
from scraper.tasks import run_long_path_scrape
from scraper.scraping_service import parse_product_html # Importar parse_product_html
...
class ScrapeURLView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            # Fast Path: Usa requests para obter o HTML
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            response = requests.get(url_to_scrape, headers=headers, timeout=10)
            ...
            # Usa a função parse_product_html do scraping_service
            nome_produto, preco_produto = parse_product_html(response.content, url_to_scrape)
            ...
            if nome_produto and preco_produto:
                ...
                return Response(
                    {"error": "Não foi possível extrair nome e preço do produto."},
